...
# Summarize predicted steps by notification type for plotting
...
ux_eng <- clean_steps_notif_users %>%
  ...
  select(user.index, decision.index, time_to_step_ge_60) %>%
  ungroup() %>%
  mutate(time_to_step_ge_60 = round(ifelse(is.na(time_to_step_ge_60) OR , 0.01, time_to_step_ge_60)), digits=2)%>%
  arrange(user.index, decision.index)
